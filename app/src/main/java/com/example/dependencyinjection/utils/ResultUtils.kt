...
fun <T, R> Flow<Result<T>>.flatMapConcatIfSuccess(transform: suspend (value: T) -> Flow<R>): Flow<Result<R>> =
    map { result -> if (result.isSuccess) { transform(result.getOrThrow()) } else { throw result.exceptionOrNull()!! } }.flattenConcat().toResult()

fun <T, R> Flow<Result<T>>.foldOnEach(onSuccess: suspend (value: T) -> R, onFailure: suspend (exception: Throwable) -> R) =
    onEach { result -> result.fold({ onSuccess.invoke(it) }, { onFailure.invoke(it) }) }
